...
void PopulationAnnealing::Result::Resize(std::size_t N) {
    energy.resize(N);
}

std::size_t PopulationAnnealing::Result::Size() {
    return energy.size();
}
...
void PopulationAnnealing::Run(std::vector<Result>& results) {
    ...
    for(auto new_beta : betalist_) {
        ...
        for(std::size_t k = 0; k < replicas_.size(); ++k) {
            ...
            observables.energy(k) = H;
        }
        // Basic observables
        observables.beta = beta_;
        observables.population = replicas_.size();
        observables.ground_energy = observables.energy.minCoeff();
        // Round-off /probably/ isn't an issue here
        observables.grounded_replicas = observables.energy.array().unaryExpr(
        ...
        double E = observables.energy.array().mean();
        std::cout 
            << beta_ << ",\t" 
            << E << ",\t" 
